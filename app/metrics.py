...
def get_product_metrics():
    ...
    total_price_cost = sum(product.cost_price * product.quantity for product in products)
    total_selling_price = sum(product.selling_price * product.quantity for product in products)
    total_quantity = sum(product.quantity for product in products)
    total_profit = total_selling_price - total_price_cost
    
    return dict(
        total_price_cost=number_format(total_price_cost, decimal_pos=2, force_grouping=True),
